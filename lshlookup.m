...
%     fprintf('%d ',b);
    ...
    bPlus = b + 1;
    if(bPlus > length(T(j).Index))
        bPlus = 1;
    end
%     bpp = bPlus + 1;
%     if(bpp > length(T(j).Index))
%         bpp = 1;
%     end
%     bppp = bpp + 1;
%     if(bppp > length(T(j).Index))
%         bppp = 1;
%     end
%     bpppp = bppp + 1;
%     if(bpppp > length(T(j).Index))
%         bpppp = 1;
%     end
    
    if (~isempty(b))
%         if(length(T(j).Index{b}) >= T(j).B)
%             iNN = [iNN T(j).Index{b} T(j).Index{bPlus} T(j).Index{bpp} T(j).Index{bppp}  T(j).Index{bpppp}];
%             iNN = [iNN T(j).Index{b} T(j).Index{bPlus} T(j).Index{bpp} T(j).Index{bppp} ];
%             iNN = [iNN T(j).Index{b} T(j).Index{bPlus} T(j).Index{bpp} ];
            iNN = [iNN T(j).Index{b} T(j).Index{bPlus} ];
%         else
%             iNN = [iNN T(j).Index{b}];
%         end
    end
